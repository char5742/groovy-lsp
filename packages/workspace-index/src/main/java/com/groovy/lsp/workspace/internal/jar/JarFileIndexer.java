...
public class JarFileIndexer {
            ...
            jarFile.entries().asIterator().forEachRemaining(entry -> {
                if (isClassFile(entry)) {
                    try {
                        ...
                        logger.debug("Error indexing class entry {}: {}", entry.getName(), e.getMessage());
                    }
                }
            });
     ...
     */
    private List<SymbolInfo> indexClassEntry(JarFile jarFile, JarEntry entry, Path jarPath) throws IOException {
        List<SymbolInfo> symbols = new ArrayList<>();
    ...
    public boolean isRelevantJar(Path jarPath) {
        ...
        try (JarFile jarFile = new JarFile(jarPath.toFile())) {
            ...
            return jarFile.entries().asIterator()
