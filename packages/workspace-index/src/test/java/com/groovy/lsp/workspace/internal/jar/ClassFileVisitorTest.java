...
class ClassFileVisitorTest {
    ...
    void testVisitMethodsAndFields() {
        ...
        // Check for fields
        long fieldCount = symbols.stream().filter(s -> s.kind() == SymbolKind.FIELD).count();
        assertThat(fieldCount).isEqualTo(2);

        // Check for constructor
        boolean hasConstructor = symbols.stream().anyMatch(s -> s.kind() == SymbolKind.CONSTRUCTOR);
        assertThat(hasConstructor).isTrue();

        // Check for methods
        long methodCount = symbols.stream().filter(s -> s.kind() == SymbolKind.METHOD).count();
    ...
    void testSkipSyntheticMethods() {
        ...
        boolean hasSynthetic = symbols.stream().anyMatch(s -> s.name().contains("synthetic"));
    ...
    private byte[] createClassWithMembers() {
        ...
        MethodVisitor method2 =
                cw.visitMethod(Opcodes.ACC_PRIVATE, "method2", "(Ljava/lang/String;)I", null, null);
        method2.visitEnd();
    ...
    private byte[] createClassWithSyntheticMethod() {
        ...
        // Add regular method
        MethodVisitor regularMethod =
                cw.visitMethod(Opcodes.ACC_PUBLIC, "regularMethod", "()V", null, null);
        regularMethod.visitEnd();

        // Add synthetic method (should be skipped)
        MethodVisitor syntheticMethod =
                cw.visitMethod(
