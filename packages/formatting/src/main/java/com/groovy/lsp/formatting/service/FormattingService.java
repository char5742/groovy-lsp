...
public class FormattingService {
    ...
    public CompletableFuture<List<TextEdit>> formatDocument(
            DocumentFormattingParams params, String documentContent) {
    ...
    public CompletableFuture<List<TextEdit>> formatRange(
            DocumentRangeFormattingParams params, String documentContent) {

        return CompletableFuture.supplyAsync(
                () -> {
                    try {
                        logger.info(
                                "Formatting range in document: {}",
                        ...
                        // Format the range
                        String formatted =
                                customFormatter.formatRange(documentContent, offset, length);
    ...
    private TextEdit createFullDocumentEdit(String original, String formatted) {
        ...
        Range fullRange = new Range(new Position(0, 0), new Position(lines - 1, lastLineLength));
    ...
    private TextEdit createRangeEdit(String original, String formatted, Range range) {
        ...
        logger.debug(
                "Creating range edit for range: start={}:{}, end={}:{}",
                range.getStart().getLine(),
                range.getStart().getCharacter(),
                range.getEnd().getLine(),
                range.getEnd().getCharacter());
