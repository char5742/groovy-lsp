...
public class CompilationResult {
    ...
    private CompilationResult(@Nullable ModuleNode moduleNode, List<CompilationError> errors, boolean successful) {
    ...
    public static class CompilationError {
        ...
        public CompilationError(String message, int line, int column, String sourceName, ErrorType type) {
            ...
            if (message instanceof SyntaxErrorMessage syntaxError) {
                ...
                if (msgText.contains("@ line")) {
                    try {
                        ...
                        if (commaIndex > 0) {
                            line = Integer.parseInt(msgText.substring(lineIndex, commaIndex).trim());
                            ...
                            if (endIndex > 0) {
                                column = Integer.parseInt(msgText.substring(columnIndex, endIndex).trim());
                            }
                        }
                    } catch (Exception e) {
